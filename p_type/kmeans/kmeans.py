...
def kmeans_data_extraction(input_file, sample_file):
    df = pd.read_csv(input_file, delimiter=";", header=None)
    df1 = pd.read_csv(sample_file)
    merge_df = pd.merge(df, df1, left_on=0, right_on='protein', how='right')
    merge_df[0] = merge_df.apply(lambda row: f"{row[0]}_{row['group']}", axis=1)
...
def perform_kmeans(processed_df):
    df_expanded = processed_df[1].str.split(',', expand=True)
    result  = pd.concat([processed_df[0], df_expanded], axis=1)
    result.columns = ['Protein Name'] + [f'Value_{i}' for i in range(len(result.columns) - 1)]
    df = result
    df[['protein_id', 'residue_type']] = df['Protein Name'].apply(lambda x: pd.Series(parse_protein_name(x)))
    df.drop(columns=['Protein Name'], inplace=True)
    # Encoding categorical variables 
    label_encoders = {}
    for column in ['protein_id', 'residue_type']:
        le = LabelEncoder()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("k-Means Clustering for p type")
    parser.add_argument('-p','--input_path', type=str, required=True, help="CSV input path with frequency")
    parser.add_argument('-s','--sample_path', type=str, required=True, help="CSV input path for sample details p53")
    args = parser.parse_args()
    processed_df = kmeans_data_extraction(args.input_path, args.sample_path)
    perform_kmeans(processed_df)
