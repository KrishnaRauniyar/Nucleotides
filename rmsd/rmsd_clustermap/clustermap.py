...
def plot_clustermap(data, residue_types, output_file, num_clusters=None):
    distance_matrix = data.astype(float)  # Use raw RMSDs

    # Compute linkage for clustering
    linkage = hierarchy.linkage(sp.distance.squareform(distance_matrix.values), method='ward')

    # Plot dendrogram for inspection
    plt.figure(figsize=(12, 6))
    hierarchy.dendrogram(linkage, labels=data.index, leaf_rotation=90)
    ...
    residue_colors = {residue: color for residue, color in zip(unique_residues, palette_residues)}
    cluster_colors = {cluster: color for cluster, color in zip(unique_clusters, palette_clusters)}
    
    row_colors_residue = [residue_colors[residue_types[i]] for i in range(len(residue_types))]
    ...
    # Create clustermap without title or legends
    g = sns.clustermap(
        distance_matrix,
        row_cluster=True,
        col_cluster=True,
